...
export function evaluateWin(grid: SymbolType[][], bet: number): WinResult[] {
  ...
  // rounded to whole cents so toasts/balance never show float garbage.
  ...
  for (let pi = 0; pi < PAYLINES.length; pi++) {
    ...
    if (contiguous >= 3 && effectiveSym !== null && !SYMBOLS[effectiveSym].scatter && !SYMBOLS[effectiveSym].bonus) {
      ...
      wins.push({ paylineIndex: pi, symbol: effectiveSym, count: contiguous, payout: Math.round(payout * lineBet * 100) / 100, positions: pos });
    }

    // Edge case: all 5 wilds — treat as diamond win
    if (contiguous === 5 && effectiveSym === null) {
      const def = SYMBOLS.diamond;
      wins.push({ paylineIndex: pi, symbol: 'diamond', count: 5, payout: Math.round(def.value5 * lineBet * 100) / 100, positions: pos });
    }
  }
  return wins;
}
